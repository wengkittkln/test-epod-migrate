...
import {updatePlateNo} from '../../../ApiController/ApiController';
import AsyncStorage from '@react-native-async-storage/async-storage';

export const useAllJobList = (route, navigation) => {
  ...
  const [orderItemListCount, setOrderItemListCount] = useState([]);
  ...
  const getJobList = () => {
    ...
    switch (joblistModel.filterType) {
      ...
      case Constants.JobType.PICK_UP:
        localPendingJobList =
          JobRealmManager.getAllJobByJobTypeSortByDecStatusAscTime(
            epodRealm,
      ...
      default:
        //All
        localPendingJobList =
          JobRealmManager.getAllJobSortByDescStatusAscSeqAscTime(epodRealm);
        break;
    }
    ...
    originalDataListRef.current = [...sortedList];
    compiledJobList();

    if (joblistModel.filterType === Constants.JobType.ALL) {
      const jobNumPayload = {
        totalJobNum: tempDataList.length,
      };
      dispatch(createAction(ActionType.SET_TOTAL_JOB_NUM, jobNumPayload));
    }
  ...
  useEffect(() => {
    AsyncStorage.getItem(Constants.PLATE_NO).then((res) => {
      ...
      if (infoModel) {
        callPlateNoApi(
          infoModel.plateNo,
          infoModel.driverName,
          infoModel.driverPhoneNumber,
          manifestData.id,
        );
      }
    });
  ...
  const callPlateNoApi = async (
    ...
    try {
      const response = await updatePlateNo(
        plateNo,
        driverName,
        driverPhoneNumber,
        ManifestId,
      ...
      console.log('updatePlateNo error:', err);
    }
  };
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      tabBarLabel: translationString.formatString(
        translationString.all_title,
        datalist.length + '/' + orderItemListCount,
      ),
    });
  }, [navigation, datalist, languageModel, orderItemListCount]);
  ...
  const getOrderItemList = () => {
    const localOrderItemList = OrderItemRealmManager.getAllOrderItem(epodRealm);
    const totalQuantity =
      OrderItemHelper.getTotalExpectedQuantity(localOrderItemList);
    setOrderItemListCount(totalQuantity);
  };
