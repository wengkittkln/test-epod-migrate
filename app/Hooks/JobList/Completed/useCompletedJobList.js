...
export const useCompletedJobList = (route, navigation) => {
  ...
  const getJobList = () => {
    ...
    switch (joblistModel.filterType) {
      ...
      default:
        //All
        localPendingJobList = JobRealmManager.getAllJobSortByPODTime(
          epodRealm,
          Constants.JobStatus.COMPLETED,
        );
        break;
    ...
    if (joblistModel.filterType === Constants.JobType.ALL) {
      const jobNumPayload = {
        completedJobNum: tempDataList.length,
      };
      dispatch(createAction(ActionType.SET_COMPLETED_JOB_NUM, jobNumPayload));
    }
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      tabBarLabel: translationString.formatString(
        translationString.completed_title,
        datalist.length,
      ),
    });
